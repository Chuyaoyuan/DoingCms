...
export class OptionsService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/options');
  ...
  update(options: IOptions): Observable<EntityResponseType> {
    return this.http.put<IOptions>(`${this.resourceUrl}/${getOptionsIdentifier(options) as number}`, options, { observe: 'response' });
  }

  partialUpdate(options: IOptions): Observable<EntityResponseType> {
    return this.http.patch<IOptions>(`${this.resourceUrl}/${getOptionsIdentifier(options) as number}`, options, { observe: 'response' });
  }
